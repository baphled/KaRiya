# Task 44: UIKit Foundation - Standardized Component Library
...
The project already has a comprehensive theme system in `internal/cli/themes/`:

| File | Purpose |
|------|---------|
| `theme.go` | `Theme` interface, `ColorPalette`, `BaseTheme` |
| `styles.go` | `StyleSet` with pre-composed styles (Buttons, Inputs, Cards, Lists, Modals, Text) |
| `default.go` | `DefaultPalette` with KaRiya colors, `NewDefaultTheme()` |
| `huh.go` | Huh library theme integration |
| `bubbles.go` | Bubbles component theming |

**UIKit will re-export and extend this system, NOT replace it.**

---

## Look and Feel (from feature/task-42-tui-architecture-refactor)

The TUI follows these visual patterns:

- **Dark theme** with muted teal accents (`#5fb3b3`)
- **Rounded borders** on all interactive elements
- **Thick borders** for focused states
- **Selection indicator**: `▶` prefix on selected items
- **Keyboard-first** design with vim-style navigation (j/k/g/G)
- **Footer badges**: `[key] action` format (e.g., `[Enter] Select  [Esc] Back`)
- **Consistent spacing**: 2-space indent, single blank lines between sections
...
| Theme system | Use existing `themes/` | Re-export `themes.Theme`, add `Aware` embeddable |
