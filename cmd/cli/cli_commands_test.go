...
var _ = Describe("CLI Command Handlers", func() {
	var (
		...
		progress    cliutil.ProgressRunner
	)
	...
	Describe("cmd.DetectBursts", func() {
		Context("when no events exist", func() {
			It("should return success with informative message", func() {
				exitCode := cmd.DetectBursts(svc, &out, &errOut, progress)
				Expect(exitCode).To(Equal(0))
				Expect(out.String()).To(ContainSubstring("No bursts detected or no events found"))
			})
		})
		...
		Context("when events exist", func() {
			...
			It("should save burst suggestions", func() {
				cmd.DetectBursts(svc, &out, &errOut, progress)
				output := out.String()
				Expect(output).To(Or(
	...
	Describe("cmd.ExtractFacts", func() {
		Context("when no events exist", func() {
			It("should return success with informative message", func() {
				exitCode := cmd.ExtractFacts(svc, &out, &errOut, progress)
				Expect(exitCode).To(Equal(0))
				Expect(out.String()).To(ContainSubstring("No facts extracted or no events found"))
			})
		})
		...
		Context("when events exist", func() {
			...
			It("should extract facts from events", func() {
				exitCode := cmd.ExtractFacts(svc, &out, &errOut, progress)
				Expect(exitCode).To(Equal(0))
				Expect(out.String()).To(ContainSubstring("Fact Extraction Results"))
			})

			It("should show extraction results", func() {
				cmd.ExtractFacts(svc, &out, &errOut, progress)
				output := out.String()
				Expect(output).To(Or(
	...
	Describe("cmd.ListBursts", func() {
		...
		Context("when no bursts exist", func() {
			It("should show no bursts message", func() {
				exitCode := cmd.ListBursts(svc, &out, &errOut)
				Expect(exitCode).To(Equal(0))
	...
	Describe("cmd.ListFacts", func() {
		...
		Context("when no facts exist", func() {
			It("should show no facts message", func() {
				exitCode := cmd.ListFacts(svc, &out, &errOut)
				Expect(exitCode).To(Equal(0))
