...
// Viewer is the interface required for modal overlay rendering.
// Any type with a View() method can be used as a modal.
type Viewer interface {
	View() string
}

// StaticViewModel is a simple Viewer that returns static content.
...
//
// Expected:
//   - modal must be non-nil and implement Viewer.
...
// Side effects:
//   - None.
func RenderModalOverlay(modal Viewer, background string) string {
...
// Side effects:
//   - None.
func RenderModalOverlayWithOffset(modal Viewer, background string, xOffset, yOffset int) string {
	bgModel := &StaticViewModel{Content: background}
	overlayModel := overlay.New(
		modal,          // Foreground: the modal
		bgModel,        // Background: the rendered view
