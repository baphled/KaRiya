...
var _ = Describe("TagSelector", func() {
	...
	Describe("Tag Selection", func() {
		...
		It("should return error when exceeding max 8 tags limit", func() {
			tags := []string{"project", "achievement", "leadership", "technical", "consulting", "research", "product", "mentoring"}
			for _, tag := range tags {
				err := selector.SelectTag(tag)
				Expect(err).NotTo(HaveOccurred())
		...
		It("should enforce max limit when all 8 tags are selected", func() {
			tags := []string{"project", "achievement", "leadership", "technical", "consulting", "research", "product", "mentoring"}
			for _, tag := range tags {
				err := selector.SelectTag(tag)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("SetSelectedTags", func() {
		It("should set selected tags from list", func() {
			selector.SetSelectedTags([]string{"technical", "leadership"})
		...
		It("should clear previous selections", func() {
			Expect(selector.SelectTag("technical")).To(Succeed())
			selector.SetSelectedTags([]string{"leadership", "product"})
			selected := selector.SelectedTags()
			Expect(selected).To(HaveLen(2))
			Expect(selected).NotTo(ContainElement("technical"))
			Expect(selected).To(ContainElement("leadership"))
			Expect(selected).To(ContainElement("product"))
		})

		It("should ignore invalid tags", func() {
		...
		It("should respect max 8 tag limit", func() {
			tags := []string{"project", "achievement", "leadership", "technical", "consulting", "research", "product", "mentoring"}
			selector.SetSelectedTags(tags)
			selected := selector.SelectedTags()
			Expect(selected).To(HaveLen(8))
		})
		...
		It("should handle all invalid tags", func() {
			selector.SetSelectedTags([]string{"invalid1", "invalid2"})
			Expect(selector.SelectedTags()).To(BeEmpty())
		})

		It("should stop adding tags at max limit", func() {
			tags := []string{"project", "achievement", "leadership", "technical", "consulting", "research", "product", "mentoring", "invalid"}
			selector.SetSelectedTags(tags)
			selected := selector.SelectedTags()
			Expect(selected).To(HaveLen(8))
		})
