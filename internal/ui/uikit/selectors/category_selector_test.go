...
var _ = Describe("CategorySelector", func() {
	...
	Describe("MapToClassificationCategories", func() {
		It("should map technical category", func() {
			result := selectors.MapToClassificationCategories([]string{"technical"})
			Expect(result).To(HaveLen(1))
		})

		It("should map leadership category", func() {
			result := selectors.MapToClassificationCategories([]string{"leadership"})
			Expect(result).To(HaveLen(1))
		})

		It("should map product category", func() {
			result := selectors.MapToClassificationCategories([]string{"product"})
			Expect(result).To(HaveLen(1))
		})

		It("should map consulting category", func() {
			result := selectors.MapToClassificationCategories([]string{"consulting"})
			Expect(result).To(HaveLen(1))
		})

		It("should map research category", func() {
			result := selectors.MapToClassificationCategories([]string{"research"})
			Expect(result).To(HaveLen(1))
		})

		It("should map mentoring category", func() {
			result := selectors.MapToClassificationCategories([]string{"mentoring"})
			Expect(result).To(HaveLen(1))
		})

		It("should map communication category", func() {
			result := selectors.MapToClassificationCategories([]string{"communication"})
			Expect(result).To(HaveLen(1))
		})

		It("should map collaboration category", func() {
			result := selectors.MapToClassificationCategories([]string{"collaboration"})
			Expect(result).To(HaveLen(1))
		})

		It("should map problem-solving category", func() {
			result := selectors.MapToClassificationCategories([]string{"problem-solving"})
			Expect(result).To(HaveLen(1))
		})

		It("should map project-management category", func() {
			result := selectors.MapToClassificationCategories([]string{"project-management"})
			Expect(result).To(HaveLen(1))
		})

		It("should map architecture category", func() {
			result := selectors.MapToClassificationCategories([]string{"architecture"})
			Expect(result).To(HaveLen(1))
		})

		It("should map multiple categories", func() {
			result := selectors.MapToClassificationCategories([]string{"technical", "leadership", "product"})
		...
		It("should ignore unknown categories", func() {
			result := selectors.MapToClassificationCategories([]string{"technical", "unknown", "leadership"})
			Expect(result).To(HaveLen(2))
		})

		It("should handle case-insensitive mapping", func() {
			result := selectors.MapToClassificationCategories([]string{"TECHNICAL", "Leadership"})
			Expect(result).To(HaveLen(2))
		})

		It("should map all 11 categories correctly", func() {
			allCategories := []string{
				"technical", "leadership", "product", "consulting", "research",
				"mentoring", "communication", "collaboration", "problem-solving",
				"project-management", "architecture",
			}
			result := selectors.MapToClassificationCategories(allCategories)
