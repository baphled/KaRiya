...
func (m *Model) handleMenuSelection() (tea.Model, tea.Cmd) {
	...
	if selectedItem.Intent == "generate_cv" {
		events, err := m.careerService.GetEventRepository().List(context.Background(), careerrepo.EventListFilters{Limit: 1})
...
func (m *Model) openConfigModal() (tea.Model, tea.Cmd) {
	...
	m.configModal = configviews.NewSettings(settings, m.width, m.height)
...
func (m *Model) saveConfigChanges() {
	...
	settings := configure.SettingsFromConfig(cfg)
	changesStruct := m.configModal.GetChanges()
	if err := configure.ApplyChanges(cfg, settings, changesStruct.Changes); err != nil {
