...
var _ = Describe("Detail", func() {
	...
	BeforeEach(func() {
		years := 5
		domainSkill := fixtures.SkillWith("skill-1", "Go Programming", "Programming Languages", "Expert")
		domainSkill.YearsUsed = &years
		skill = display.SkillFromDomain(domainSkill)
		theme = themes.NewDefaultTheme()
	})

	Describe("NewDetailModal", func() {
		It("creates a modal with the skill and theme", func() {
			modal = skillview.NewDetail(skill, theme, 10, nil)
			Expect(modal).NotTo(BeNil())
			Expect(modal.IsVisible()).To(BeFalse())
		})

		It("creates a modal with event count", func() {
			modal = skillview.NewDetail(skill, theme, 25, nil)
			Expect(modal).NotTo(BeNil())
		})
	...
	Describe("View", func() {
		...
		Context("when visible", func() {
			...
			It("handles nil theme by using default", func() {
				nilThemeModal := skillview.NewDetail(skill, nil, 10, nil)
			...
			It("renders event count when zero", func() {
				modal = skillview.NewDetail(skill, theme, 0, nil)
				modal.SetDimensions(120, 40)
				modal.Show()
				view := modal.View()
				Expect(view).NotTo(BeEmpty())
		...
		Context("with nil years used", func() {
			BeforeEach(func() {
				skillWithoutYears := display.SkillFromDomain(fixtures.SkillWith("skill-2", "Python", "Languages", "Intermediate"))
				modal = skillview.NewDetail(skillWithoutYears, theme, 5, nil)
				modal.SetDimensions(120, 40)
				modal.Show()
			})
	...
	Describe("SetSkill", func() {
		...
		It("updates the skill being displayed", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-new", "Rust", "Systems", "Beginner"))
		...
		It("handles skill with nil years used", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-nil-years", "Python", "Scripting", "Intermediate"))
		...
		It("handles skill with 1 year (singular)", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-one-year", "Ruby", "Web", "Advanced"))
			oneYear := 1
		...
		It("handles skill with multiple years (plural)", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-many-years", "JavaScript", "Frontend", "Expert"))
			manyYears := 10
		...
		It("handles skill with empty category", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-no-cat", "TypeScript", "", "Intermediate"))
			modal.SetSkill(newSkill, 0, nil)
			modal.Show()
		...
		It("handles skill with empty level", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-no-level", "Java", "Backend", ""))
			modal.SetSkill(newSkill, 0, nil)
			modal.Show()
		...
		It("handles skill with zero event count", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-no-events", "Ruby", "Web", "Advanced"))
			modal.SetSkill(newSkill, 0, nil)
			modal.Show()
	...
	Describe("GetSkillID", func() {
		...
		It("returns correct ID for different skills", func() {
			newSkill := display.SkillFromDomain(fixtures.SkillWith("skill-999", "Rust", "Systems", "Expert"))
			modal = skillview.NewDetail(newSkill, theme, 5, nil)
			Expect(modal.GetSkillID()).To(Equal("skill-999"))
		})
	})
})
