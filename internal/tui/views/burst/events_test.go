...
var _ = Describe("Events", func() {
	...
	BeforeEach(func() {
		...
		events = []display.Event{
			display.EventFromDomain(e1),
			display.EventFromDomain(e2),
			display.EventFromDomain(e3),
		}
		theme = themes.NewDefaultTheme()
	})

	Describe("NewEvents", func() {
		It("creates modal with events content", func() {
			modal = burstview.NewEvents(burstID, burstName, events, theme)
		...
		It("handles nil theme with default", func() {
			modal = burstview.NewEvents(burstID, burstName, events, nil)

			Expect(modal).NotTo(BeNil())
		})

		It("handles empty events list", func() {
			modal = burstview.NewEvents(burstID, burstName, []display.Event{}, theme)
		...
		It("handles nil events list", func() {
			modal = burstview.NewEvents(burstID, burstName, nil, theme)
	...
	Describe("SetEvents", func() {
		It("updates the displayed events", func() {
			...
			newEvents := []display.Event{
				display.EventFromDomain(ne1),
				display.EventFromDomain(ne2),
			}
		...
		It("updates event count in view", func() {
			...
			modal.SetEvents([]display.Event{display.EventFromDomain(se)})
			view = modal.View()
			Expect(view).To(ContainSubstring("1"))
		})
	})
