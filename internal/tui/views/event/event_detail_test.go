...
var _ = Describe("Detail", func() {
	var (
		modal *eventview.Detail
		event display.Event
		theme themes.Theme
	)

	BeforeEach(func() {
		domainEvent := fixtures.EventWith("test-event-id", "Event description text", "Test Company", "Test Project")
	...
	Describe("NewDetail", func() {
		It("creates modal with event content", func() {
			modal = eventview.NewDetail(event, theme)
		...
		It("handles nil theme with default", func() {
			modal = eventview.NewDetail(event, nil)
	...
	Describe("SetEvent", func() {
		It("updates the displayed event", func() {
			modal = eventview.NewDetail(event, theme)
			newDomainEvent := fixtures.Event("new-id")
			newDomainEvent.Text = "New event text"
			newEvent := display.EventFromDomain(newDomainEvent)
...
var _ = Describe("SkillsDetail", func() {
	...
	BeforeEach(func() {
		...
		skills = display.SkillsFromDomain([]*career.Skill{goSkill, dockerSkill})
		theme = themes.NewDefaultTheme()
	})

	Describe("NewSkillsDetail", func() {
		It("creates modal with skills content", func() {
			modal = eventview.NewSkillsDetail(eventID, skills, theme)
		...
		It("handles nil theme with default", func() {
			modal = eventview.NewSkillsDetail(eventID, skills, nil)

			Expect(modal).NotTo(BeNil())
		})

		It("handles empty skills list", func() {
			modal = eventview.NewSkillsDetail(eventID, []display.Skill{}, theme)

			Expect(modal).NotTo(BeNil())
		})

		It("handles nil skills list", func() {
			modal = eventview.NewSkillsDetail(eventID, nil, theme)
