...
func (i *Intent) wizardView() string {
	...
	case StateReview:
		if i.activeView != nil {
			return i.renderReviewViewWithModalOverlay(width, height)
		}
		content = "Loading review..."
	case StatePreview:
		if i.activeView != nil {
			return i.renderPreviewViewWithModalOverlay(width, height)
		}
...
func (i *Intent) renderReviewViewWithModalOverlay(width, height int) string {
	breadcrumbs := i.getWizardBreadcrumbs()
	view := intents.CreateStandardViewWithBreadcrumbs(i.BaseIntent, breadcrumbs...)

	content := i.activeView.RenderContent()
	view.WithContent(content)
...
func (i *Intent) renderPreviewViewWithModalOverlay(width, height int) string {
	breadcrumbs := i.getWizardBreadcrumbs()
	view := intents.CreateStandardViewWithBreadcrumbs(i.BaseIntent, breadcrumbs...)
