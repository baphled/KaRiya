...
// IntentValidator is the input context passed to the GenerateCV intent.
//
// Side effects:
//   - None.
...
// Side effects:
//   - None.
func (ctx *IntentValidator) Validate() error {
	if ctx == nil {
		return errors.New("IntentValidator cannot be nil")
	}

	if len(ctx.AvailableProfiles) == 0 {
		return errors.New("IntentValidator must have at least one available profile")
	}

	if len(ctx.Events) == 0 {
		return errors.New("IntentValidator must have at least one event")
	}
