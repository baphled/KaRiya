...
var _ = Describe("Intent Methods", func() {
	...
	Describe("View", func() {
		...
		It("should show no bursts found when empty in list", func() {
			emptyCtx := &burst_management.IntentValidator{
				Bursts: []*career.Burst{},
			}
			emptyCtx.Validate()
	...
	Describe("Delete Burst Flow", func() {
		...
		It("should delete burst when 'y' is pressed in delete confirm state", func() {
			...
			// Press 'y' to confirm deletion (now returns async cmd).
			intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})

			// Process the async delete result.
			intent.Update(burst_management.BurstDeletedMsg{BurstID: burst.ID})
		...
		It("should handle confirm action from detail screen", func() {
			...
			// Press 'c' for confirm (now returns async cmd).
			intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'c'}})

			// Process the async facts load to create confirm modal.
			intent.Update(burst_management.ConfirmBurstFactsLoadedMsg{Facts: []*career.Fact{}})
	...
	Describe("Confirm Burst Flow", func() {
		...
		It("should mark burst as confirmed when 'y' is pressed in confirm state", func() {
			...
			// Press 'y' to confirm (now returns async cmd).
			cmd := intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})
			Expect(cmd).NotTo(BeNil(), "confirmBurst should return a command")

			// Process the async confirm result.
			burst.Confirmed = true
			intent.Update(burst_management.BurstConfirmedMsg{Burst: burst})
	...
	Describe("BurstEditCompleteMsg Handler", func() {
		...
		Context("when LoadBursts fails", func() {
			...
			It("should show detail modal when edit completes successfully", func() {
				msg := burst_management.BurstEditCompleteMsg{
					Burst: burst,
					Error: nil,
				}
		...
		Context("when edit completes successfully", func() {
			It("should show detail modal for edited burst", func() {
				msg := burst_management.BurstEditCompleteMsg{
					Burst: burst,
					Error: nil,
				}
	...
	Describe("Suggestion Modal Message Flow", func() {
		...
		It("should send SuggestionReviewCompleteMsg when modal closes with accepted suggestions", func() {
			...
			// Accept suggestion (returns async cmd for saving burst).
			cmd := intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'a'}})
			Expect(cmd).NotTo(BeNil())

			// Execute the async save command and process the result.
			msg := cmd()
			intent.Update(msg)

			// Verify burst was created via async BurstSavedMsg handler.
	...
	Describe("HandleCancel State Transitions", func() {
		...
		It("should ignore cancel when in StateList with detail modal active", func() {
			...
			navResult := &widgets.NavigateViewResult{ResultData: burst}
		...
		It("should return to list and clear delete modal from StateDeleteConfirm", func() {
			...
			navResult := &widgets.NavigateViewResult{ResultData: actionData}
			intent.HandleNavigate(navResult)
			Expect(intent.HasVisibleDeleteModal()).To(BeTrue())
	...
	Describe("Service Integration Tests", func() {
		...
		It("should handle EditBurstMsg with valid update", func() {
			...
			_ = mockRepo.Create(context.Background(), burst)
	...
	Describe("Context CRUD with Repository", func() {
		...
		It("should create burst with repository", func() {
			...
			bursts, _ := repo.List(context.Background(), *fixtures.BurstListFilters())
		...
		It("should update burst with repository", func() {
			...
			// Verify update.
			updated, _ := repo.GetByID(context.Background(), "burst-1")
		...
		It("should load bursts from repository", func() {
			// Create some bursts.
			_ = repo.Create(context.Background(), fixtures.Burst("b1", "e1", "e2"))
			_ = repo.Create(context.Background(), fixtures.Burst("b2", "e3", "e4"))
	...
	Describe("Delete and Confirm Flows with Repository", func() {
		...
		It("should delete burst through confirm modal", func() {
			...
			// Confirm deletion with 'y' (returns batch cmd with async delete).
			cmd := intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})
			Expect(cmd).NotTo(BeNil())

			// Execute all batch messages to trigger the async repo delete.
		...
		It("should handle delete error gracefully", func() {
			...
			_ = repo.Delete(context.Background(), burst.ID)
			...
			// Try to confirm deletion - returns async cmd.
			intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})

			// Process the async BurstDeletedMsg with error.
			intent.Update(burst_management.BurstDeletedMsg{BurstID: burst.ID, Error: errors.New("burst not found")})
		...
		It("should confirm burst and trigger fact extraction", func() {
			...
			// Navigate to detail and confirm (async facts load).
			navResult := &widgets.NavigateViewResult{ResultData: burst}
			intent.HandleNavigate(navResult)
			intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'c'}})
			intent.Update(burst_management.ConfirmBurstFactsLoadedMsg{Facts: []*career.Fact{}})

			// Confirm with 'y' (returns async confirmBurst cmd).
			intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})

			// Process the async BurstConfirmedMsg (triggers fact extraction).
			burst.Confirmed = true
			cmd := intent.Update(burst_management.BurstConfirmedMsg{Burst: burst})
		...
		It("should handle confirm error gracefully", func() {
			...
			// Try to confirm (returns async confirmBurst cmd).
			intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})

			// Process the async BurstConfirmedMsg with error.
			intent.Update(burst_management.BurstConfirmedMsg{Burst: burst, Error: fmt.Errorf("database unavailable")})
	...
	Describe("Async Command Execution", func() {
		...
		It("should execute showEvents command", func() {
			// Navigate to detail.
			navResult := &widgets.NavigateViewResult{ResultData: burst}
			intent.HandleNavigate(navResult)
		...
		It("should execute showFacts command", func() {
			testFacts := []*career.Fact{
				fixtures.FactWith("f1", "Fact 1"),
			}
			mockService.SetFactsForBurst(burst.ID, testFacts)
	...
	Describe("Fact Extraction", func() {
		...
		It("should execute extractFactsForBurst and save facts via suggestion acceptance", func() {
			...
			// Create a suggestion and accept it - this triggers async save.
			...
			// Press 'a' to accept - returns async save cmd (BurstSavedMsg).
			cmd := intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'a'}})
			Expect(cmd).NotTo(BeNil())
			...
			// Should be BurstSavedMsg (async save).
			savedMsg, ok := msg.(burst_management.BurstSavedMsg)
			Expect(ok).To(BeTrue())
			Expect(savedMsg.Error).ToNot(HaveOccurred())

			// Process BurstSavedMsg which chains to extractFactsForBurst.
			extractCmd := intent.Update(savedMsg)
			Expect(extractCmd).NotTo(BeNil())

			// Execute the extraction command.
			extractMsg := extractCmd()
			extractionMsg, ok := extractMsg.(burst_management.FactExtractionCompleteMsg)
			Expect(ok).To(BeTrue())
		...
		It("should handle extractFactsForBurst with extraction error via suggestion acceptance", func() {
			...
			// Press 'a' to accept (returns async save cmd).
			cmd := intent.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'a'}})
			Expect(cmd).NotTo(BeNil())
			...
			// Execute the extraction command — should return error.
			extractMsg := extractCmd()
			extractionMsg, ok := extractMsg.(burst_management.FactExtractionCompleteMsg)
			Expect(ok).To(BeTrue())
		...
		It("should handle extractFactsForBurst with save error via suggestion acceptance (skips failed saves)", func() {
			...
			Expect(ok).To(BeTrue())
			Expect(extractionMsg.Error).ToNot(HaveOccurred())
			Expect(extractionMsg.Facts).To(BeEmpty())
		})
		...
		It("should show re-extract confirmation when facts already exist", func() {
			...
			// Process the async ConfirmBurstFactsLoadedMsg with existing facts.
			intent.Update(burst_management.ConfirmBurstFactsLoadedMsg{Facts: existingFacts})
