//go:generate mockgen -destination=../../../testutil/mocks/intent/skills_skill_inference_service_mock.go -package=mockintent -mock_names=SkillInferenceService=MockSkillsSkillInferenceService github.com/baphled/kariya/internal/tui/intents/skillsmanagement SkillInferenceService
...
// IntentValidator holds input parameters and business logic for the ManageSkills intent.
...
// NewIntentValidator constructs an IntentValidator wired to the given repository with default empty filters.
//
// Expected: ctx must be a non-nil context.Context; skillRepo must be a non-nil SkillRepository.
//
// Returns: a fully initialized IntentValidator ready for Validate and use by NewIntent.
//
// Side effects: None.
func NewIntentValidator(ctx context.Context, skillRepo career.SkillRepository) *IntentValidator {
...
func (c *IntentValidator) LoadSkills() ([]*domain.Skill, error) {
	...
	return c.SkillRepository.List(context.Background(), repoFilters)
}
...
//
// Side effects: performs a repository read operation against the underlying data store.
func (c *IntentValidator) GetEventCounts() (map[string]int, error) {
	return c.SkillRepository.GetEventCountsForSkills(context.Background())
}
...
//
// Side effects: performs a repository read operation against the underlying data store.
func (c *IntentValidator) GetEventsForSkill(skillID string) ([]*domain.Event, error) {
	return c.SkillRepository.GetEventsUsingSkill(context.Background(), skillID)
}
