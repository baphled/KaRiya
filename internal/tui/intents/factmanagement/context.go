...
// IntentValidator holds input parameters and business logic for the FactManagement intent.
...
// NewIntentValidator initialises a FactManagement context with sensible defaults for pagination, sorting, and filtering.
//
// Expected: ctx must be non-nil. factRepo may be nil for offline/test usage, but repository operations will fail.
//
// Returns: a fully initialised IntentValidator ready for use by the intent.
//
// Side effects: None.
func NewIntentValidator(ctx context.Context, factRepo careerrepo.FactRepository) *IntentValidator {
	_ = ctx
...
// Side effects:
//   - None.
func (c *IntentValidator) LoadFacts() error {
	if c.FactRepository == nil {
		return ErrServiceNotAvailable
	}
	facts, err := c.FactRepository.List(context.Background(), careerrepo.FactListFilters{})
	if err != nil {
		return err
	}
	c.Facts = facts
...
// Side effects:
//   - None.
func (c *IntentValidator) GetPageFacts() []*domain.Fact {
	if len(c.Facts) == 0 {
...
// Side effects:
//   - None.
func (c *IntentValidator) SelectFact(index int) {
...
// Side effects:
//   - None.
func (c *IntentValidator) GetSelectedFact() *domain.Fact {
...
// Side effects:
//   - None.
func (c *IntentValidator) ClearFormErrors() {
	c.FormErrors = make(map[string]string)
}
...
// Side effects:
//   - None.
func (c *IntentValidator) SetFormError(field, message string) {
...
// Side effects:
//   - None.
func (c *IntentValidator) HasFormErrors() bool {
...
func (c *IntentValidator) CreateFact(fact *domain.Fact) error {
	if c.FactRepository == nil {
		return ErrServiceNotAvailable
	}
	...
	if err := c.FactRepository.Create(context.Background(), fact); err != nil {
		return err
	}
...
func (c *IntentValidator) UpdateFact(fact *domain.Fact) error {
	if c.FactRepository == nil {
		return ErrServiceNotAvailable
	}
	if err := fact.Validate(); err != nil {
	...
	if err := c.FactRepository.Update(context.Background(), fact); err != nil {
		return err
	}
...
func (c *IntentValidator) DeleteFact(factID string) error {
	if c.FactRepository == nil {
		return ErrServiceNotAvailable
	}
	if err := c.FactRepository.Delete(context.Background(), factID); err != nil {
		return err
	}
	for i, f := range c.Facts {
		if f.ID == factID {
...
// Side effects:
//   - None.
func (c *IntentValidator) StartNewFact() {
...
// Side effects:
//   - None.
func (c *IntentValidator) StartEditFact(fact *domain.Fact) {
	c.EditingFact = &domain.Fact{
		ID:                   fact.ID,
		Text:                 fact.Text,
...
// Side effects:
//   - None.
func (c *IntentValidator) SaveEdit() error {
...
// Side effects:
//   - None.
func (c *IntentValidator) ToggleRowExpansion(rowIndex int) {
...
//
// Side effects: None.
func (c *IntentValidator) IsRowExpanded(rowIndex int) bool {
