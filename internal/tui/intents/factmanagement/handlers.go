...
func (i *Intent) handleListKeyActions(msg tea.KeyMsg) tea.Cmd {
	...
	case "n":
		...
		i.editModal = factviews.NewEditFact(display.FactFromDomain(i.context.EditingFact))
		i.state = StateEditor
...
func (i *Intent) handleRefreshKey() tea.Cmd {
	if err := i.context.LoadFacts(); err != nil {
		...
		i.tableBehavior.SetItems(display.FactsFromDomain(i.context.Facts))
		i.syncTableSelection()
	}
	return nil
}
...
func (i *Intent) handleEditorModalResult(result *factviews.EditResult) {
	if result.Accepted {
...
func (i *Intent) applyEditorChanges(result *factviews.EditResult) bool {
	...
	i.context.EditingFact.RoleFit = career.RoleFit(result.Modified.RoleFit)
