...
//
// Convention (ADR: Modal Business Logic Separation):
//   - T MUST be a plain DTO struct with primitive fields (string, bool, int, []string).
//   - T MUST NOT contain domain entities (career.Skill, career.Event, etc.).
//   - T MUST NOT import domain or service packages.
//   - Business logic (validation, entity construction) belongs in domain packages.
//   - Intent handlers map T fields to domain functions via tea.Cmd.
