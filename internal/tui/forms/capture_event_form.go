...
const (
	...
	eventDescriptionHint        = "Describe what you accomplished (required, 10-2000 characters)"
	eventDescriptionPlaceholder = "Enter event description..."
	dateFormatHint              = "YYYY-MM-DD, 'today', or relative like '1 week ago'"
	defaultsToTodayHint         = "Defaults to today"
	...
	if strategy == "quick" {
		fields = []huh.Field{
			NewText(FieldConfig{
				...
				Description: eventDescriptionHint,
			...
			NewInput(FieldConfig{
				Key:         "date",
				Title:       "Date",
				Description: dateFormatHint,
				Placeholder: defaultsToTodayHint,
				Validate:    DateFormat,
			}).Value(&data.Date),
		}
	} else {
