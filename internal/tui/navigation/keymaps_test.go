...
var _ = Describe("KeyMaps", func() {
	...
	Describe("ListKeyMap", func() {
		...
		Describe("Sort binding", func() {
			It("should match 's' key", func() {
				Expect(key.Matches(keyMsg("s"), keyMap.Sort)).To(BeTrue())
			})

			It("should have help text", func() {
				help := keyMap.Sort.Help()
				Expect(help.Key).To(Equal("s"))
				Expect(help.Desc).To(Equal("sort"))
			})
		})

		Describe("Clear binding", func() {
			It("should match 'x' key", func() {
				Expect(key.Matches(keyMsg("x"), keyMap.Clear)).To(BeTrue())
			})

			It("should have help text", func() {
				help := keyMap.Clear.Help()
				Expect(help.Key).To(Equal("x"))
				Expect(help.Desc).To(Equal("clear filters"))
			})
		})
