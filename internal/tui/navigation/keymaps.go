...
type ListKeyMap struct {
	...
	ShowSkills  key.Binding // 'k' for show event skills
	InferSkills key.Binding // 'i' for infer skills
}

// defaultNavigationBindings returns a ListKeyMap with navigation key bindings.
//
// Returns:
//   - A ListKeyMap value with navigation fields populated.
//
// Side effects:
//   - None.
func defaultNavigationBindings() ListKeyMap {
	return ListKeyMap{
		Up: key.NewBinding(
			key.WithKeys("up", "k"),
...
// defaultActionBindings populates action key bindings in a ListKeyMap.
//
// Returns:
//   - A ListKeyMap value with action fields populated.
//
// Side effects:
//   - None.
func defaultActionBindings(km ListKeyMap) ListKeyMap {
	...
	km.Clear = key.NewBinding(
		key.WithKeys("x"),
		key.WithHelp("x", "clear filters"),
	)
	km.ShowSkills = key.NewBinding(
		key.WithKeys("K"),
		key.WithHelp("K", "show skills"),
	)
	km.InferSkills = key.NewBinding(
		key.WithKeys("I"),
		key.WithHelp("I", "infer skills"),
	)
	return km
}
...
func DefaultListKeyMap() ListKeyMap {
	return defaultActionBindings(defaultNavigationBindings())
}
...
func (k ListKeyMap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		...
		{k.Select, k.Add, k.Edit, k.Delete, k.Filter, k.Search, k.Sort, k.Clear},
	}
}
