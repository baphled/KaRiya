...
var _ = Describe("CVPreviewScreen", func() {
	...
	Describe("Highlight rendering", func() {
		...
		It("should use audience relevance for non-master audience with populated map", func() {
			...
			bullet.AudienceRelevance = map[string]float64{"hiring_manager": 0.95}
			group := fixtures.ContentGroupWithBullets("Engineering", []*career.CVBullet{bullet})
			section := fixtures.CVSectionWithContent("s1", "cv-ar", []*career.SectionContentGroup{group})
			section.Title = "Experience"

			arCV := fixtures.CVViewWith("cv-ar", "AR CV", "Engineer", "hiring_manager")
		...
		It("should fall back to confidence when audience key not in map", func() {
			...
			fbCV := fixtures.CVViewWith("cv-fb", "Fallback CV", "Engineer", "hiring_manager")
		...
		It("orders highlights by audience relevance score descending", func() {
			lowBullet := fixtures.CVBulletWith("b-low", "s1", "Low relevance preview item")
			lowBullet.AudienceRelevance = map[string]float64{"hiring_manager": 0.1}
			lowBullet.Confidence = 1.0

			highBullet := fixtures.CVBulletWith("b-high", "s1", "High relevance preview item")
			...
			orderCV := fixtures.CVViewWith("cv-order", "Order CV", "Engineer", "hiring_manager")
			orderCV.Sections = []*career.CVSection{section}

			orderScreen := cv.NewCVPreviewScreen(orderCV)
			orderScreen.Update(tea.WindowSizeMsg{Width: 80, Height: 40})
			view := orderScreen.View()

			highIdx := strings.Index(view, "High relevance preview item")
			lowIdx := strings.Index(view, "Low relevance preview item")
		...
		It("produces different ordering for different audiences", func() {
			techBullet := fixtures.CVBulletWith("b-tech", "s1", "Architected scalable system")
			...
			bizBullet := fixtures.CVBulletWith("b-biz", "s1", "Delivered major revenue impact")
			...
			hmCV := fixtures.CVViewWith("cv-hm", "HM CV", "Engineer", "hiring_manager")
			hmCV.Sections = []*career.CVSection{section}

			hmScreen := cv.NewCVPreviewScreen(hmCV)
			hmScreen.Update(tea.WindowSizeMsg{Width: 80, Height: 40})
			hmView := hmScreen.View()

			hmBizIdx := strings.Index(hmView, "Delivered major revenue impact")
			hmTechIdx := strings.Index(hmView, "Architected scalable system")
			...
			recCV := fixtures.CVViewWith("cv-rec", "Recruiter CV", "Engineer", "recruiter")
			recCV.Sections = []*career.CVSection{recruiterSection}

			recScreen := cv.NewCVPreviewScreen(recCV)
			recScreen.Update(tea.WindowSizeMsg{Width: 80, Height: 40})
			recView := recScreen.View()

			recBizIdx := strings.Index(recView, "Delivered major revenue impact")
			recTechIdx := strings.Index(recView, "Architected scalable system")
