...
var _ = Describe("SuggestionReviewModal (Fact)", func() {
	var (
	...
	Describe("Creation", func() {
		It("should create modal with facts and theme", func() {
			modal = modals.NewFactSuggestionModal(facts, theme)
		...
		It("should create modal with nil theme using default", func() {
			modal = modals.NewFactSuggestionModal(facts, nil)
			Expect(modal).NotTo(BeNil())
			view := modal.View()
		...
		It("should handle empty facts list", func() {
			modal = modals.NewFactSuggestionModal([]career.Fact{}, theme)
		...
		It("should handle single fact", func() {
			singleFact := []career.Fact{facts[0]}
			modal = modals.NewFactSuggestionModal(singleFact, theme)
			Expect(modal.HasSuggestions()).To(BeTrue())
