...
func (i *Intent) handleBurstSuggestionsLoaded(msg BurstSuggestionsLoadedMsg) tea.Cmd {
	...
	// Convert burst_fact.BurstSuggestion to BurstSuggestion.
	i.suggestions = make([]BurstSuggestion, len(msg.Suggestions))
	for idx, s := range msg.Suggestions {
		i.suggestions[idx] = BurstSuggestion{
			EventIDs:        s.EventIDs,
...
// acceptSuggestion creates a burst from an accepted suggestion.
func (i *Intent) acceptSuggestion(suggestion BurstSuggestion) tea.Cmd {
	// Create new burst from suggestion.
	burst := &career.Burst{
