...
|-------|-----------|-------------|
| **Per-package** (modified packages) | >= 95% | Pre-commit hook (BLOCKING) |
| **Project average** | >= 80% | `make check-compliance` (warning) |

**How to Check Coverage**:
```bash
# Check specific package coverage
go test -cover ./internal/cli/intents/generatecv/...

# Check multiple packages
go test -cover ./internal/cli/screens/cv/... ./internal/cli/screens/cv/modals/...

# Find low-coverage functions in a package
go test -coverprofile=/tmp/cover.out ./internal/cli/intents/generatecv/... && \
  go tool cover -func=/tmp/cover.out | grep -v "100.0%"
```

**Coverage Violations are BLOCKING**: The pre-commit hook will reject commits if any modified package has < 95% coverage.
