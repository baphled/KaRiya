...
For high-level, business-focused acceptance tests, use Godog (Gherkin/Cucumber):

```bash
make bdd           # Run all BDD tests
make bdd-smoke     # Run @smoke tests only
make bdd-wip       # Run @wip tests only
```

**Feature files location:** `features/*.feature`

```gherkin
@onboarding @smoke
Scenario: Complete full onboarding wizard
...
**📖 BDD guide:** [features/README.md](features/README.md)
