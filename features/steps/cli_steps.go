...
func RegisterCLISteps(sc *godog.ScenarioContext) {
	...
	sc.Step(`^I should see the event in the system$`, cliEventShouldBeSaved)
	sc.Step(`^I should see confirmation message$`, cliShouldSeeConfirmation)
	sc.Step(`^I should see the event with all metadata in the system$`, cliEventShouldBeSavedWithMetadata)
	...
	sc.Step(`^the system is initialized$`, cliDatabaseShouldBeCreated)
	sc.Step(`^migrations should be applied$`, cliMigrationsShouldBeApplied)
	sc.Step(`^I should see system status$`, cliShouldSeeDatabaseStatus)
